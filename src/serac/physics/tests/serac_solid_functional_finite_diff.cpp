...
TEST(solid_functional_finite_diff, finite_difference_shape)
{
  ...
  // Compute the sensitivity (d QOI/ d state * d state/d parameter) given the current adjoint solution
  [[maybe_unused]] auto& sensitivity = solid_solver.computeSensitivity<SHAPE_FIELD>();
  ...
  for (int i = 0; i < user_defined_shape_displacement.gridFunc().Size(); ++i) {
    ...
    SLIC_INFO(axom::fmt::format("sensitivity: {}", sensitivity.trueVec()(i)));
    EXPECT_NEAR((sensitivity.trueVec()(i) - dqoi_dshape) / std::max(dqoi_dshape, 1.0e-2), 0.0, 3.5e-3);
  }
}
